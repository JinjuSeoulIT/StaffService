...
public class StaffController {
    ...
    public ResponseEntity<ApiResponse<List<StaffEntity>>> openStaffsList()
    {
        List<StaffEntity> list = staffService.selectStaffList();
...
//        list.forEach(s -> System.out.println(s.getId()));
        ...
        return ResponseEntity.ok(
                new ApiResponse<List<StaffEntity>>().ok(list)
                                );
    }
    ...
    @GetMapping(produces="application/json; charset=UTF-8")
    public ResponseEntity<ApiResponse<StaffEntity>> open321StaffDetail(int id) throws Exception
    {
